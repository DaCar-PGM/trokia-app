...
# --- CONFIGURATION PRO ---
st.set_page_config(page_title="Trokia v16 : Gold Edition", page_icon="🏆", layout="wide")

# Headers rotatifs (simulés) pour passer inaperçu
HEADERS = {
...
# --- 1. FONCTIONS ROBUSTES (ANTI-CRASH) ---

def clean_price(text):
    """Extrait un prix proprement depuis n'importe quel texte bordélique"""
    try:
        # Cherche 10,50 ou 10.50
        found = re.findall(r"(\d+[\.,]?\d*)", text)
        for f in found:
            val = float(f.replace(",", "."))
            # Filtre de bon sens : pas de prix à 0.1€ ni à 100000€
            if 1 < val < 10000:
                return val
    except: pass
    return None
...
def analyser_image_multi(image_pil, modele):
    try:
        ...
        # Prompt optimisé pour la rapidité et la précision
        prompt = (
...
def get_thumbnail(query):
    """Récupère image eBay (compatible Code Barre EAN et Mots Clés)"""
    try:
        ...
        for item in items:
            ...
            if img_tag:
                ...
                # On veut une vraie image eBay, pas un gif
                if cand and 'ebayimg.com' in cand: return cand
    except: pass
    return "https://via.placeholder.com/150x150.png?text=Image+Non+Trouvée"

# --- 2. MOTEURS DE RECHERCHE BLINDÉS ---

def generer_lien_google(nom, site):
    return f"https://www.google.com/search?q=site:{site}+{nom.replace(' ', '+')}"

def scan_smart_ddg(nom, site):
    """Utilise DuckDuckGo pour scanner le web sans blocage"""
    try:
        query = f"site:{site} {nom}"
        # On utilise le backend 'lite' ou 'html' qui est plus rapide
        ...
        # En cas d'erreur silencieuse, on renvoie 0
        return 0, 0, generer_lien_google(nom, site)

def scan_ebay_ean_or_text(recherche):
    """Fonctionne aussi bien pour un nom 'Playstation' que pour un EAN '339189199...'"""
    try:
        clean = re.sub(r'[^\w\s]', '', recherche).strip()
        # URL spécifique eBay qui marche pour tout
...
# --- UI ---
st.title("🏆 Trokia v16 : Gold Edition")
...
def reset_all():
    ...
    st.toast("Mémoire effacée ! Prêt pour le prochain scan.")
...
with col_logo:
    st.caption("L'outil de revente intelligent par Armand")
with col_btn:
        ...
        if st.session_state.current_img != f.name:
            ...
            with st.spinner("🤖 L'IA analyse l'objet..."):
                ...
                if p: 
                    ...
                    # Préchargement des images de réf
...
# --- TAB 2 : MANUEL / EAN ---
with tab_manuel:
    st.info("💡 Astuce : Scannez un Code Barre (EAN) pour une précision 100%.")
...
if st.session_state.go_search and st.session_state.nom_final:
    ...
    # 3. Vinted (Uniquement si pertinent ou si EAN pas trouvé)
    vp, vn, vu = 0, 0, generer_lien_google(st.session_state.nom_final, "vinted.fr")
    # Si c'est un code barre ou des fringues, on regarde vinted
    ...
    # Affichage Image (Si EAN, eBay trouve souvent la photo officielle)
    if ei:
        col_i1, col_i2 = st.columns([1, 4])
        col_i1.image(ei, width=150, caption="Produit identifié")
    ...
    # Prix suggéré intelligent (eBay en prio, sinon la médiane des autres)
    ...
    c3.metric("Marge Nette (Poche)", f"{marge:.2f} €", delta="Gagnant" if marge > 0 else "Perdant")

    if st.button("💾 Enregistrer dans le Stock", use_container_width=True):
        if sheet:
            img_save = ei if ei else "https://via.placeholder.com/150"
            sheet.append_row([datetime.now().strftime("%d/%m %H:%M"), st.session_state.nom_final, vente, achat, f"{marge:.2f}", img_save])
            st.balloons() # L'effet WAOUH pour la famille
